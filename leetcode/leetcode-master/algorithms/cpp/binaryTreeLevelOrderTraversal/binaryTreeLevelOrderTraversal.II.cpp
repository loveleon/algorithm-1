...
void levelOrderBottom2(TreeNode *root)  
{  
    vector<TreeNode*> vec;
    vector<int> num_per_level;
    ...
    {//循环截止条件：没有更多层了  
		...
		num_per_level.push_back(cur);
        //cout<<']'<<endl;//该层访问结束，输出换行
    }

	//cout<<num_per_level.size()<<endl;
	int i = num_per_level.size() - 1;
	int j = 0;
	for (i; i >= 0; i--)
	{
		if (i < 0)
